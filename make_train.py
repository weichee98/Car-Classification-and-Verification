...
logger = Logger()
logger.log('Start make_train.py')
...
parser.add_argument('-tb', '--train_batch_size', type=int, default=100)
...
if args.model_config:
    ...
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dense(163, activation='softmax', kernel_regularizer=tf.keras.regularizers.l2(l=1e-3))
    ], name='inception_v3_make_classifier5')
...
train_ds = pipeline.load_dataset(train_df['filename'], train_df['make_id'], batch_size=train_batch_size, mode='train')
TRAIN_STEPS = len(train_df) // train_batch_size

if args.valid_data:
    valid_batch_size = args.valid_batch_size
    valid_ds = pipeline.load_dataset(valid_df['filename'], valid_df['make_id'], batch_size=valid_batch_size, mode='valid')
